...
function filteredInsects() {
  ...
  return insects.filter((insect) => {
    ...
    return (!query || terms.includes(query)) && (state.diet === "all" || insect.diet === state.diet) && (state.level === "all" || String(insect.familiarityLevel) === state.level);
  });
}
...
function bindEvents() {
  ...
  $$("#dietFilter button").forEach((button) => button.addEventListener("click", () => { state.diet = button.dataset.diet; setPressed("#dietFilter button", "active", "diet", state.diet); renderCatalog(); }));
  $$("#levelFilter button").forEach((button) => button.addEventListener("click", () => { state.level = button.dataset.level; setPressed("#levelFilter button", "active", "level", state.level); renderCatalog(); }));
  $("#resetFilters").addEventListener("click", () => { state.query = ""; state.diet = "all"; state.level = "all"; $("#searchInput").value = ""; setPressed("#dietFilter button", "active", "diet", "all"); setPressed("#levelFilter button", "active", "level", "all"); renderCatalog(); });
...
bindEvents(); updateMetrics(); renderCatalog();
