...
const newFriendExtraExpansionIds = new Set(["cerura-felina", "chrysiridia-rhipheus", "theraphosa-blondi", "mymaridae", "dipentium-japonicum", "pulex-irritans", "monomorium-chinense", "brephidium-exilis", "eriophyes-tiliae", "lissachatina-fulica", "limax-flavus", "maratus-volans", "bombyx-mori", "tenebrio-molitor"]);
...
if ([...newFriendRepresentativeIds].some((id) => {
  ...
  return !insect || insect.gallery.length !== (newFriendExpansionIds.has(id) ? 2 + Number(newFriendExtraExpansionIds.has(id)) : 1) || insect.reviewStatus !== "gallery-published-pending-user-review" || insect.gallery[0].role !== "생태 대표 관찰" || insect.familiarityLevel !== 1 || !insect.region.includes("새로운 친구");
