...
if (!Array.isArray(insects) || insects.length !== 47) throw new Error("Production insects data must contain the expected public information records.");
...
if (insects.some((insect) => insect.gallery.length && insect.gallery.length !== 4)) throw new Error("Every illustrated public species must contain exactly four registered gallery images.");
if (publicGalleryItems.length < 144 || publicGalleryItems.length !== insects.filter((insect) => insect.gallery.length).length * 4 || publicGalleryItems.some((item) => !item.src?.startsWith("assets/insects/approved/") || !item.license || !item.generationPrompt || !item.generationSeed || !item.generationWorkflow || !["approved", "published-pending-user-review"].includes(item.reviewStatus))) throw new Error("Public galleries must contain only fully recorded registered assets.");
