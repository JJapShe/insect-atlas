...
const appIconRecord = JSON.parse(await readFile(new URL("../tools/generation-tests/app-icon-20260910.json", import.meta.url), "utf8"));
if (appIconRecord.assets?.length !== 2 || ![appIconRecord.source, appIconRecord.license, appIconRecord.generationPrompt, appIconRecord.generationSeed, appIconRecord.generationWorkflow, appIconRecord.reviewStatus].every(Boolean) || appIconRecord.assets.some((asset) => !webManifest.icons.some((icon) => icon.src === asset))) throw new Error("Mobile app-icon provenance is incomplete.");
...
const newFriendExpansionIds = new Set(["cerura-felina", "chrysiridia-rhipheus", "theraphosa-blondi", "mymaridae", "dipentium-japonicum", "pulex-irritans", "monomorium-chinense", "brephidium-exilis", "eriophyes-tiliae", "lissachatina-fulica", "limax-flavus", "maratus-volans", "greta-oto", "sphaerocoris-annulus", "catoxantha-purpurea", "polymita-picta", "bombyx-mori", "tenebrio-molitor"]);
const newFriendExtraExpansionIds = new Set(["cerura-felina", "chrysiridia-rhipheus", "theraphosa-blondi", "mymaridae", "dipentium-japonicum", "pulex-irritans", "monomorium-chinense", "brephidium-exilis", "eriophyes-tiliae", "lissachatina-fulica", "limax-flavus", "maratus-volans", "greta-oto", "sphaerocoris-annulus", "catoxantha-purpurea", "polymita-picta", "bombyx-mori", "tenebrio-molitor"]);
...
if (!Array.isArray(insects) || insects.length !== 142 || new Set(insects.map((insect) => insect.id)).size !== 142 || [...familiarLocalIds, ...rareFamousIds, ...familiarNextIds, ...newFriendRepresentativeIds, ...priorityRepresentativeIds].some((id) => !insects.some((insect) => insect.id === id))) throw new Error("Production data must contain 142 unique records, including priority representative images.");
...
if (insects.some((insect) => insect.gallery.length && ![4, 5, 6, 7].includes(insect.gallery.length) && !((newFriendRepresentativeIds.has(insect.id) || priorityRepresentativeIds.has(insect.id)) && [1, 2, 3].includes(insect.gallery.length)))) throw new Error("Illustrated species must retain four standard images, except staged representative galleries.");
...
const acustaLifeStages = insects.find((item) => item.id === "acusta-despecta");
if (!acustaLifeStages || acustaLifeStages.gallery.length !== 6 || !["egg", "juvenile"].every((stage) => acustaLifeStages.gallery.some((item) => item.src.endsWith(`acusta-despecta-${stage}-imagegen-v1.png`)))) throw new Error("Acusta despecta life-stage gallery is incomplete.");
for (const stage of ["egg", "juvenile"]) {
  const fileName = `acusta-despecta-${stage}-imagegen-v1.png`;
  const reviewCopy = await readFile(new URL(`../assets/insects/review/priority-life-cycle-20260922-f/${fileName}`, import.meta.url));
  const publicCopy = await readFile(new URL(`../assets/insects/approved/${fileName}`, import.meta.url));
  if (!reviewCopy.equals(publicCopy)) throw new Error(`Review and public copies differ: ${fileName}`);
}
...
const chrysinaGallery = insects.find((item) => item.id === "chrysina-gloriosa");
{
  const fileName = "chrysina-gloriosa-ecology-imagegen-v1.png";
  if (!chrysinaGallery || chrysinaGallery.gallery.length !== 4 || !chrysinaGallery.gallery.some((item) => item.src.endsWith(fileName))) throw new Error("Chrysina gloriosa ecology gallery is incomplete.");
  const reviewCopy = await readFile(new URL(`../assets/insects/review/priority-gallery-20260926-a/${fileName}`, import.meta.url));
  const publicCopy = await readFile(new URL(`../assets/insects/approved/${fileName}`, import.meta.url));
  if (!reviewCopy.equals(publicCopy)) throw new Error(`Review and public copies differ: ${fileName}`);
}
...
const deroplatysGallery = insects.find((item) => item.id === "deroplatys-desiccata");
{
  const fileName = "deroplatys-desiccata-ecology-imagegen-v1.png";
  if (!deroplatysGallery || deroplatysGallery.gallery.length !== 4 || !deroplatysGallery.gallery.some((item) => item.src.endsWith(fileName))) throw new Error("Deroplatys desiccata ecology gallery is incomplete.");
  const reviewCopy = await readFile(new URL(`../assets/insects/review/priority-gallery-20260926-c/${fileName}`, import.meta.url));
  const publicCopy = await readFile(new URL(`../assets/insects/approved/${fileName}`, import.meta.url));
  if (!reviewCopy.equals(publicCopy)) throw new Error(`Review and public copies differ: ${fileName}`);
}
const ranatraGallery = insects.find((item) => item.id === "ranatra-chinensis");
{
  const fileName = "ranatra-chinensis-ecology-imagegen-v1.png";
  if (!ranatraGallery || ranatraGallery.gallery.length !== 4 || !ranatraGallery.gallery.some((item) => item.src.endsWith(fileName))) throw new Error("Ranatra chinensis ecology gallery is incomplete.");
  const reviewCopy = await readFile(new URL(`../assets/insects/review/priority-gallery-20260926-d/${fileName}`, import.meta.url));
  const publicCopy = await readFile(new URL(`../assets/insects/approved/${fileName}`, import.meta.url));
  if (!reviewCopy.equals(publicCopy)) throw new Error(`Review and public copies differ: ${fileName}`);
}
...
if (new Set(newFriendExpansionManifest.species?.map((entry) => entry.id)).size !== newFriendExpansionIds.size || newFriendExpansionManifest.species.some((entry) => !newFriendExpansionIds.has(entry.id) || !entry.kind || !entry.asset || !entry.supports)) throw new Error("New-friend gallery expansion provenance manifest is incomplete.");
